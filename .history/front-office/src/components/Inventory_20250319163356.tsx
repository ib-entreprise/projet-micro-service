...
const Inventory = () => {
  ...
  useEffect(() => {
    const fetchInventory = async () => {
      try {
        const response = await fetch('http://localhost:9091/api/i/inventory'); // Via le gateway
  ...
  const handleAddItem = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('http://localhost:9091/api/i/inventory', {
        method: 'POST',
  ...
  const handleDeleteItem = async (id) => {
    try {
      await fetch(`http://localhost:9091/api/i/inventory/${id}`, {
        method: 'DELETE',
      });
